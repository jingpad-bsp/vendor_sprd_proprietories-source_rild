...
Return<void> RadioConfigImpl::setResponseFunctions(
        const ::android::sp<V1_0::IRadioConfigResponse>& radioConfigResponse,
        const ::android::sp<V1_0::IRadioConfigIndication>& radioConfigIndication) {
    pthread_rwlock_t *radioServiceRwlockPtr = radio::getRadioServiceRwlock(RIL_SOCKET_1);
...
Return<void> RadioConfigImpl::setSimSlotsMapping(int32_t serial, const hidl_vec<uint32_t>& slotMap) {
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, RIL_SOCKET_1, RIL_REQUEST_CONFIG_SET_SLOT_MAPPING);
    if (pRI == NULL) {
        return Void();
    }
    ...
    for (size_t socket_id = 0; socket_id < slotNum; socket_id++) {
        ...
        for (size_t nextId = socket_id + 1; nextId < slotNum; nextId++) {
            if (slotMap[socket_id] == slotMap[nextId]) {
                RLOGE("setSimSlotsMapping: slot parameter is the same:[%zu] and [%zu]", socket_id, nextId);
                sendErrorResponse(pRI, RIL_E_INVALID_ARGUMENTS);
                return Void();
            }
        }
    ...
    bool isChangedMapping = false;
    isChangedMapping = isChangeSlotId((int)RIL_SOCKET_1 , slotMap[RIL_SOCKET_1]);
    ...
    if (!isChangedMapping) {
        RLOGD("It does not need to change slot mapping");
        free(pSlotMap);
        ...
        REQUEST_INFO_PACK(pRI, cb, pSlotMap, slotNum * sizeof(int));
        CALL_ONREQUEST(RIL_REQUEST_CONFIG_SET_SLOT_MAPPING, pSlotMap, slotNum * sizeof(int), pRI,
                       pRI->socket_id);
    }
...
Return<void> RadioConfigImpl::setModemsConfig(int32_t serial, const V1_1::ModemsConfig& modemsConfig) {
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_CONFIG_SET_MODEM_CONFIG);
    if (pRI == NULL) {
        return Void();
    }

    RIL_ModemConfig *mdConfig = (RIL_ModemConfig *)calloc(1, sizeof(RIL_ModemConfig));
    if (mdConfig == NULL) {
    ...
    REQUEST_INFO_PACK(pRI, cb, mdConfig, sizeof(RIL_ModemConfig));

    CALL_ONREQUEST(RIL_REQUEST_CONFIG_SET_MODEM_CONFIG, mdConfig, sizeof(RIL_ModemConfig), pRI, pRI->socket_id);
...
void radio::registerConfigService(RIL_RadioFunctions *callbacks, CommandInfo *commands) {
    ...
    int slotId = rilSlotMapping(RIL_SOCKET_1);
...
void checkReturnStatus(Return<void>& ret) {
    if (ret.isOk() == false) {
        ...
        pthread_rwlock_t *radioServiceRwlockPtr = radio::getRadioServiceRwlock(0);
...
int radio::getSimSlotsStatusResponse(int slotId, int responseType, int serial,
...
int radio::setSimSlotsMappingResponse(int slotId, int responseType, int serial,
                                      RIL_Errno e, void *response, size_t responseLen) {
#if VDBG
...
int radio::getPhoneCapabilityResponse(int slotId, int responseType, int serial,
                                      RIL_Errno e, void *response, size_t responseLen) {
#if VDBG
...
int radio::setPreferredDataModemResponse(int slotId, int responseType, int serial,
                                         RIL_Errno e, void *response, size_t responseLen) {
#if VDBG
...
int radio::setModemsConfigResponse(int slotId, int responseType, int serial,
                                   RIL_Errno e, void *response, size_t responseLen) {
#if VDBG
...
int radio::getModemsConfigResponse(int slotId, int responseType, int serial,
                                   RIL_Errno e, void *response, size_t responseLen) {
#if VDBG
...
int radio::simSlotsStatusChanged(int slotId, int indicationType, int token, RIL_Errno e,
