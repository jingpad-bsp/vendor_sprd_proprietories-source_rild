...
typedef enum {
    ...
    RIL_E_UNISOC_BASE = 600,
    RIL_E_INVALID_PARAMETER = RIL_E_UNISOC_BASE + 1,
} RIL_Errno;
...
typedef enum {
    ...
    RADIO_TECH_NR = 20  /* 5G NR. This is only use in 5G Standalone mode. */
} RIL_RadioTechnology;
...
typedef enum {
    ...
    /* the following definitions are extended in radio/1.4 */
    RAF_NR = (1 << RADIO_TECH_NR)  /* 5G NR. This is only use in 5G Standalone mode. */
...
typedef struct {
    ...
    char logicalModemUuid[UUID_MAX_LENGTH]; // A UUID typically "com.xxxx.lmX where X is the logical modem.
...
typedef enum {
    ...
    CALL_FAIL_OEM_CAUSE_3 = 0xf003, /* Emergency temp call fail */
    CALL_FAIL_OEM_CAUSE_4 = 0xf004, /* Emergency prem call fail */
...
typedef enum {
    ...
    PDP_FAIL_LLC_SNDCP = 0x19,                     /* Network cannot provide the requested service and PDP context is
                                                    * deactivated because of LLC or SNDCP failure.*/
    ...
    /* the following definitions are extended in radio/1.4 */
        PDP_FAIL_ACTIVATION_REJECTED_BCM_VIOLATION          = 0x30,     /* UE requested to modify QoS parameters or the bearer control mode,
                                                                         * which is not compatible with the selected bearer control mode.*/
        PDP_FAIL_COLLISION_WITH_NETWORK_INITIATED_REQUEST   = 0x38,     /* Network has already initiated the activation, modification, or
                                                                         * deactivation of bearer resources that was requested by the UE. */
        PDP_FAIL_ONLY_IPV4V6_ALLOWED                        = 0x39,     /* Network supports IPv4v6 PDP type only. Non-IP type is not allowed.
                                                                         * In LTE mode of operation, this is a PDN throttling cause code,
                                                                         * meaning the UE may throttle further requests to the same APN. */

        PDP_FAIL_ONLY_NON_IP_ALLOWED                        = 0x3A,     /* Network supports non-IP PDP type only. IPv4, IPv6 and IPv4v6 is not allowed.
                                                                         * In LTE mode of operation, this is a PDN throttling cause code, meaning the
                                                                         * UE can throttle further requests to the same APN. */
        PDP_FAIL_UNSUPPORTED_QCI_VALUE                      = 0x3B,     /* QCI (QoS Class Identifier) indicated in the UE request cannot be supported. */
        PDP_FAIL_BEARER_HANDLING_NOT_SUPPORTED              = 0x3C,     /* Procedure requested by the UE was rejected because the bearer handling is not supported. */
        PDP_FAIL_INVALID_DNS_ADDR                           = 0x7B,     /* Not receiving a DNS address that was mandatory.*/
        PDP_FAIL_INVALID_PCSCF_OR_DNS_ADDRESS               = 0x7C,     /* Not receiving either a PCSCF or a DNS address, one of them being mandatory. */
        PDP_FAIL_CALL_PREEMPT_BY_EMERGENCY_APN              = 0x7F,     /* Emergency call bring up on a different ePDG. */
        PDP_FAIL_UE_INITIATED_DETACH_OR_DISCONNECT          = 0x80,     /* UE performs a detach or disconnect PDN action based on TE requirements. */
        PDP_FAIL_MIP_FA_REASON_UNSPECIFIED                  = 0x7D0,    /* Reason unspecified for foreign agent rejected MIP (Mobile IP) registration. */
        PDP_FAIL_MIP_FA_ADMIN_PROHIBITED                    = 0x7D1,    /* Foreign agent administratively prohibited MIP (Mobile IP) registration. */
        PDP_FAIL_MIP_FA_INSUFFICIENT_RESOURCES              = 0x7D2,    /* Foreign agent rejected MIP (Mobile IP) registration because of insufficient resources. */
        PDP_FAIL_MIP_FA_MOBILE_NODE_AUTHENTICATION_FAILURE  = 0x7D3,    /* Foreign agent rejected MIP (Mobile IP) registration because of MN-AAA authenticator was wrong. */
        PDP_FAIL_MIP_FA_HOME_AGENT_AUTHENTICATION_FAILURE   = 0x7D4,    /* Foreign agent rejected MIP (Mobile IP) registration because of home agent authentication failure. */
        PDP_FAIL_MIP_FA_REQUESTED_LIFETIME_TOO_LONG         = 0x7D5,    /* Foreign agent rejected MIP (Mobile IP) registration because of requested lifetime was too long. */
        PDP_FAIL_MIP_FA_MALFORMED_REQUEST                   = 0x7D6,    /* Foreign agent rejected MIP (Mobile IP) registration because of malformed request.*/
        PDP_FAIL_MIP_FA_MALFORMED_REPLY                     = 0x7D7,    /* Foreign agent rejected MIP (Mobile IP) registration because of malformed reply. */
        PDP_FAIL_MIP_FA_ENCAPSULATION_UNAVAILABLE           = 0x7D8,    /* Foreign agent rejected MIP (Mobile IP) registration because of requested encapsulation was * unavailable. */
        PDP_FAIL_MIP_FA_VJ_HEADER_COMPRESSION_UNAVAILABLE   = 0x7D9,    /* Foreign agent rejected MIP (Mobile IP) registration of VJ Header Compression was unavailable. */
        PDP_FAIL_MIP_FA_REVERSE_TUNNEL_UNAVAILABLE          = 0x7DA,    /* Foreign agent rejected MIP (Mobile IP) registration because of reverse tunnel was unavailable. */
        PDP_FAIL_MIP_FA_REVERSE_TUNNEL_IS_MANDATORY         = 0x7DB,    /* Foreign agent rejected MIP (Mobile IP) registration because of reverse tunnel was mandatory but not requested by device. */
        PDP_FAIL_MIP_FA_DELIVERY_STYLE_NOT_SUPPORTED        = 0x7DC,    /* Foreign agent rejected MIP (Mobile IP) registration because of delivery style was not supported. */
        PDP_FAIL_MIP_FA_MISSING_NAI                         = 0x7DD,    /* Foreign agent rejected MIP (Mobile IP) registration because of missing NAI (Network Access Identifier). */
        PDP_FAIL_MIP_FA_MISSING_HOME_AGENT                  = 0x7DE,    /* Foreign agent rejected MIP (Mobile IP) registration because of missing Home Agent. */
        PDP_FAIL_MIP_FA_MISSING_HOME_ADDRESS                = 0x7DF,    /* Foreign agent rejected MIP (Mobile IP) registration because of missing Home Address. */
        PDP_FAIL_MIP_FA_UNKNOWN_CHALLENGE                   = 0x7E0,    /* Foreign agent rejected MIP (Mobile IP) registration because of unknown challenge. */
        PDP_FAIL_MIP_FA_MISSING_CHALLENGE                   = 0x7E1,    /* Foreign agent rejected MIP (Mobile IP) registration because of missing challenge. */
        PDP_FAIL_MIP_FA_STALE_CHALLENGE                     = 0x7E2,    /* Foreign agent rejected MIP (Mobile IP) registration because of stale challenge. */
        PDP_FAIL_MIP_HA_REASON_UNSPECIFIED                  = 0x7E3,    /* Reason unspecified for home agent rejected MIP (Mobile IP) registration. */
        PDP_FAIL_MIP_HA_ADMIN_PROHIBITED                    = 0x7E4,    /* Home agent administratively prohibited MIP (Mobile IP) registration. */
        PDP_FAIL_MIP_HA_INSUFFICIENT_RESOURCES              = 0x7E5,    /* Home agent rejected MIP (Mobile IP) registration because of insufficient resources. */
        PDP_FAIL_MIP_HA_MOBILE_NODE_AUTHENTICATION_FAILURE  = 0x7E6,    /* Home agent rejected MIP (Mobile IP) registration because of MN-HA authenticator was wrong. */
        PDP_FAIL_MIP_HA_FOREIGN_AGENT_AUTHENTICATION_FAILURE = 0x7E7,   /* Home agent rejected MIP (Mobile IP) registration because of foreign agent authentication failure. */
        PDP_FAIL_MIP_HA_REGISTRATION_ID_MISMATCH            = 0x7E8,    /* Home agent rejected MIP (Mobile IP) registration because of registration id mismatch. */
        PDP_FAIL_MIP_HA_MALFORMED_REQUEST                   = 0x7E9,    /* Home agent rejected MIP (Mobile IP) registration because of malformed request. */
        PDP_FAIL_MIP_HA_UNKNOWN_HOME_AGENT_ADDRESS          = 0x7EA,    /* Home agent rejected MIP (Mobile IP) registration because of unknown home agent address. */
        PDP_FAIL_MIP_HA_REVERSE_TUNNEL_UNAVAILABLE          = 0x7EB,    /* Home agent rejected MIP (Mobile IP) registration because of reverse tunnel was unavailable. */
        PDP_FAIL_MIP_HA_REVERSE_TUNNEL_IS_MANDATORY         = 0x7EC,    /* Home agent rejected MIP (Mobile IP) registration because of reverse tunnel is mandatory but not requested by device. */
        PDP_FAIL_MIP_HA_ENCAPSULATION_UNAVAILABLE           = 0x7ED,    /* Home agent rejected MIP (Mobile IP) registration because of encapsulation unavailable. */
        PDP_FAIL_CLOSE_IN_PROGRESS                          = 0x7EE,    /* Tearing down is in progress. */
        PDP_FAIL_NETWORK_INITIATED_TERMINATION              = 0x7EF,    /* Brought down by the network. */
        PDP_FAIL_MODEM_APP_PREEMPTED                        = 0x7F0,    /* Another application in modem preempts the data call. */
        PDP_FAIL_PDN_IPV4_CALL_DISALLOWED                   = 0x7F1,    /* IPV4 PDN is in throttled state due to network providing only IPV6 address during the previous
                                                                         * VSNCP bringup (subs_limited_to_v6). */
        PDP_FAIL_PDN_IPV4_CALL_THROTTLED                    = 0x7F2,    /* IPV4 PDN is in throttled state due to previous VSNCP bringup failure(s). */
        PDP_FAIL_PDN_IPV6_CALL_DISALLOWED                   = 0x7F3,    /* IPV6 PDN is in throttled state due to network providing only IPV4 address during the previous
                                                                         * VSNCP bringup (subs_limited_to_v4). */
        PDP_FAIL_PDN_IPV6_CALL_THROTTLED                    = 0x7F4,    /* IPV6 PDN is in throttled state due to previous VSNCP bringup failure(s). */
        PDP_FAIL_MODEM_RESTART                              = 0x7F5,    /* Modem restart. */
        PDP_FAIL_PDP_PPP_NOT_SUPPORTED                      = 0x7F6,    /* PDP PPP calls are not supported. */
        PDP_FAIL_UNPREFERRED_RAT                            = 0x7F7,    /* RAT on which the data call is attempted/connected is no longer the preferred RAT. */
        PDP_FAIL_PHYSICAL_LINK_CLOSE_IN_PROGRESS            = 0x7F8,    /* Physical link is in the process of cleanup. */
        PDP_FAIL_APN_PENDING_HANDOVER                       = 0x7F9,    /* Interface bring up is attempted for an APN that is yet to be handed over to target RAT. */
        PDP_FAIL_PROFILE_BEARER_INCOMPATIBLE                = 0x7FA,    /* APN bearer type in the profile does not match preferred network mode. */
        PDP_FAIL_SIM_CARD_CHANGED                           = 0x7FB,    /* Card was refreshed or removed. */
        PDP_FAIL_LOW_POWER_MODE_OR_POWERING_DOWN            = 0x7FC,    /* Device is going into lower power mode or powering down. */
        PDP_FAIL_APN_DISABLED                               = 0x7FD,    /* APN has been disabled. */
        PDP_FAIL_MAX_PPP_INACTIVITY_TIMER_EXPIRED           = 0x7FE,    /* Maximum PPP inactivity timer expired. */
        PDP_FAIL_IPV6_ADDRESS_TRANSFER_FAILED               = 0x7FF,    /* IPv6 address transfer failed. */
        PDP_FAIL_TRAT_SWAP_FAILED                           = 0x800,    /* Target RAT swap failed. */
        PDP_FAIL_EHRPD_TO_HRPD_FALLBACK                     = 0x801,    /* Device falls back from eHRPD to HRPD. */
        PDP_FAIL_MIP_CONFIG_FAILURE                         = 0x802,    /* UE is in MIP-only configuration but the MIP configuration fails on call bring up due to incorrect provisioning. */
        PDP_FAIL_PDN_INACTIVITY_TIMER_EXPIRED               = 0x803,    /* PDN inactivity timer expired due to no data transmission in a configurable duration of time. */
        PDP_FAIL_MAX_IPV4_CONNECTIONS                       = 0x804,    /* IPv4 data call bring up is rejected because the UE already maintains the allotted maximum number of IPv4 data connections. */
        PDP_FAIL_MAX_IPV6_CONNECTIONS                       = 0x805,    /* IPv6 data call bring up is rejected because the UE already maintains the allotted maximum number of IPv6 data connections. */
        PDP_FAIL_APN_MISMATCH                               = 0x806,    /* New PDN bring up is rejected during interface selection because the UE has already allotted the available interfaces for other PDNs. */
        PDP_FAIL_IP_VERSION_MISMATCH                        = 0x807,    /* New call bring up is rejected since the existing data call IP type doesn't match the requested IP. */
        PDP_FAIL_DUN_CALL_DISALLOWED                        = 0x808,    /* Dial up networking (DUN) call bring up is rejected since UE is in eHRPD RAT. */
        PDP_FAIL_INTERNAL_EPC_NONEPC_TRANSITION             = 0x809,    /* Rejected/Brought down since UE is transition between EPC and NONEPC RAT. */
        PDP_FAIL_INTERFACE_IN_USE                           = 0x80A,    /* The current interface is being in use. */
        PDP_FAIL_APN_DISALLOWED_ON_ROAMING                  = 0x80B,    /* PDN connection to the APN is disallowed on the roaming network. */
        PDP_FAIL_APN_PARAMETERS_CHANGED                     = 0x80C,    /* APN-related parameters are changed. */
        PDP_FAIL_NULL_APN_DISALLOWED                        = 0x80D,    /* PDN is attempted to be brought up with NULL APN but NULL APN is not supported. */
        PDP_FAIL_THERMAL_MITIGATION                         = 0x80E,    /* Thermal level increases and causes calls to be torn down when normal mode of operation is not allowed. */
        PDP_FAIL_DATA_SETTINGS_DISABLED                     = 0x80F,    /* PDN Connection to a given APN is disallowed because data is disabled from the device user interface settings. */
        PDP_FAIL_DATA_ROAMING_SETTINGS_DISABLED             = 0x810,    /* PDN Connection to a given APN is disallowed because data roaming is disabled from the device user interface settings and the UE is roaming. */
        PDP_FAIL_DDS_SWITCHED                               = 0x811,    /* DDS (Default data subscription) switch occurs. */
        PDP_FAIL_FORBIDDEN_APN_NAME                         = 0x812,    /* PDN being brought up with an APN that is part of forbidden APN Name list. */
        PDP_FAIL_DDS_SWITCH_IN_PROGRESS                     = 0x813,    /* Default data subscription switch is in progress. */
        PDP_FAIL_CALL_DISALLOWED_IN_ROAMING                 = 0x814,    /* Roaming is disallowed during call bring up. */
        PDP_FAIL_NON_IP_NOT_SUPPORTED                       = 0x815,    /* UE is unable to bring up a non-IP data call because the device is not camped on a NB1 cell. */
        PDP_FAIL_PDN_NON_IP_CALL_THROTTLED                  = 0x816,    /* Non-IP PDN is in throttled state due to previous VSNCP bringup failure(s). */
        PDP_FAIL_PDN_NON_IP_CALL_DISALLOWED                 = 0x817,    /* Non-IP PDN is in disallowed state due to the network providing only an IP address. */
        PDP_FAIL_CDMA_LOCK                                  = 0x818,    /* Device in CDMA locked state. */
        PDP_FAIL_CDMA_INTERCEPT                             = 0x819,    /* Received an intercept order from the base station. */
        PDP_FAIL_CDMA_REORDER                               = 0x81A,    /* Receiving a reorder from the base station. */
        PDP_FAIL_CDMA_RELEASE_DUE_TO_SO_REJECTION           = 0x81B,    /* Receiving a release from the base station with a SO (Service Option) Reject reason. */
        PDP_FAIL_CDMA_INCOMING_CALL                         = 0x81C,    /* Receiving an incoming call from the base station. */
        PDP_FAIL_CDMA_ALERT_STOP                            = 0x81D,    /* Received an alert stop from the base station due to incoming only. */
        PDP_FAIL_CHANNEL_ACQUISITION_FAILURE                = 0x81E,    /* Channel acquisition failures. This indicates that device has failed acquiring all the channels in the PRL. */
        PDP_FAIL_MAX_ACCESS_PROBE                           = 0x81F,    /* Maximum access probes transmitted. */
        PDP_FAIL_CONCURRENT_SERVICE_NOT_SUPPORTED_BY_BASE_STATION = 0x820,    /* Concurrent service is not supported by base station. */
        PDP_FAIL_NO_RESPONSE_FROM_BASE_STATION              = 0x821,    /* There was no response received from the base station. */
        PDP_FAIL_REJECTED_BY_BASE_STATION                   = 0x822,    /* The base station rejecting the call. */
        PDP_FAIL_CONCURRENT_SERVICES_INCOMPATIBLE           = 0x823,    /* The concurrent services requested were not compatible. */
        PDP_FAIL_NO_CDMA_SERVICE                            = 0x824,    /* Device does not have CDMA service. */
        PDP_FAIL_RUIM_NOT_PRESENT                           = 0x825,    /* RUIM not being present. */
        PDP_FAIL_CDMA_RETRY_ORDER                           = 0x826,    /* Receiving a retry order from the base station. */
        PDP_FAIL_ACCESS_BLOCK                               = 0x827,    /* Access blocked by the base station. */
        PDP_FAIL_ACCESS_BLOCK_ALL                           = 0x828,    /* Access blocked by the base station for all mobile devices. */
        PDP_FAIL_IS707B_MAX_ACCESS_PROBES                   = 0x829,    /* Maximum access probes for the IS-707B call. */
        PDP_FAIL_THERMAL_EMERGENCY                          = 0x82A,    /* Put device in thermal emergency. */
        PDP_FAIL_CONCURRENT_SERVICES_NOT_ALLOWED            = 0x82B,    /* In favor of a voice call or SMS when concurrent voice and data are not supported. */
        PDP_FAIL_INCOMING_CALL_REJECTED                     = 0x82C,    /* The other clients rejected incoming call. */
        PDP_FAIL_NO_SERVICE_ON_GATEWAY                      = 0x82D,    /* No service on the gateway. */
        PDP_FAIL_NO_GPRS_CONTEXT                            = 0x82E,    /* GPRS context is not available. */
        PDP_FAIL_ILLEGAL_MS                                 = 0x82F,    /* Network refuses service to the MS because either an identity of the MS is not acceptable
                                                                         * to the network or the MS does not pass the authentication check. */
        PDP_FAIL_ILLEGAL_ME                                 = 0x830,    /* * ME could not be authenticated and the ME used is not acceptable to the network. */
        PDP_FAIL_GPRS_SERVICES_AND_NON_GPRS_SERVICES_NOT_ALLOWED = 0x831,    /* Not allowed to operate either GPRS or non-GPRS services. */
        PDP_FAIL_GPRS_SERVICES_NOT_ALLOWED                  = 0x832,    /* MS is not allowed to operate GPRS services. */
        PDP_FAIL_MS_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK = 0x833,  /* No matching identity or context could be found in the network. */
        PDP_FAIL_IMPLICITLY_DETACHED                        = 0x834,    /* Mobile reachable timer has expired, or the GMM context data related to the subscription does not exist in the SGSN. */
        PDP_FAIL_PLMN_NOT_ALLOWED                           = 0x835,    /* UE requests GPRS service, or the network initiates a detach request in a PLMN which does not offer roaming for GPRS services to that MS. */
        PDP_FAIL_LOCATION_AREA_NOT_ALLOWED                  = 0x836,    /* MS requests service, or the network initiates a detach request, in a location area where the
                                                                         * HPLMN determines that the MS, by subscription, is not allowed to operate. */
        PDP_FAIL_GPRS_SERVICES_NOT_ALLOWED_IN_THIS_PLMN     = 0x837,    /* UE requests GPRS service or the network initiates a detach request in a PLMN that does not offer roaming for GPRS services. */
        PDP_FAIL_PDP_DUPLICATE                              = 0x838,    /* PDP context already exists. */
        PDP_FAIL_UE_RAT_CHANGE                              = 0x839,    /* RAT change on the UE. */
        PDP_FAIL_CONGESTION                                 = 0x83A,    /* Network cannot serve a request from the MS due to congestion. */
        PDP_FAIL_NO_PDP_CONTEXT_ACTIVATED                   = 0x83B,    /* MS requests an establishment of the radio access bearers for all active PDP contexts by sending a service request message
                                                                         * indicating data to the network, but the SGSN does not have any active PDP context. */
        PDP_FAIL_ACCESS_CLASS_DSAC_REJECTION                = 0x83C,    /* Access class blocking restrictions for the current camped cell. */
        PDP_FAIL_PDP_ACTIVATE_MAX_RETRY_FAILED              = 0x83D,    /* SM attempts PDP activation for a maximum of four attempts. */
        PDP_FAIL_RADIO_ACCESS_BEARER_FAILURE                = 0x83E,    /* Radio access bearer failure. */
        PDP_FAIL_ESM_UNKNOWN_EPS_BEARER_CONTEXT             = 0x83F,    /* Invalid EPS bearer identity in the request. */
        PDP_FAIL_DRB_RELEASED_BY_RRC                        = 0x840,    /* Data radio bearer is released by the RRC. */
        PDP_FAIL_CONNECTION_RELEASED                        = 0x841,    /* Indicate the connection was released. */
        PDP_FAIL_EMM_DETACHED                               = 0x842,    /* UE is detached. */
        PDP_FAIL_EMM_ATTACH_FAILED                          = 0x843,    /* Attach procedure is rejected by the network. */
        PDP_FAIL_EMM_ATTACH_STARTED                         = 0x844,    /* Attach procedure is started for EMC purposes. */
        PDP_FAIL_LTE_NAS_SERVICE_REQUEST_FAILED             = 0x845,    /* Service request procedure failure. */
        PDP_FAIL_DUPLICATE_BEARER_ID                        = 0x846,    /* Active dedicated bearer was requested using the same default bearer ID. */
        PDP_FAIL_ESM_COLLISION_SCENARIOS                    = 0x847,    /* Collision scenarios for the UE and network-initiated procedures. */
        PDP_FAIL_ESM_BEARER_DEACTIVATED_TO_SYNC_WITH_NETWORK = 0x848,   /* Bearer must be deactivated to synchronize with the network. */
        PDP_FAIL_ESM_NW_ACTIVATED_DED_BEARER_WITH_ID_OF_DEF_BEARER = 0x849,    /* Active dedicated bearer was requested for an existing default bearer. */
        PDP_FAIL_ESM_BAD_OTA_MESSAGE                        = 0x84A,    /* Bad OTA message is received from the network. */
        PDP_FAIL_ESM_DOWNLOAD_SERVER_REJECTED_THE_CALL      = 0x84B,    /* Download server rejected the call. */
        PDP_FAIL_ESM_CONTEXT_TRANSFERRED_DUE_TO_IRAT        = 0x84C,    /* PDN was disconnected by the downlaod server due to IRAT. */
        PDP_FAIL_DS_EXPLICIT_DEACTIVATION                   = 0x84D,    /* Dedicated bearer will be deactivated regardless of the network response. */
        PDP_FAIL_ESM_LOCAL_CAUSE_NONE                       = 0x84E,    /* No specific local cause is mentioned, usually a valid OTA cause. */
        PDP_FAIL_LTE_THROTTLING_NOT_REQUIRED                = 0x84F,    /* Throttling is not needed for this service request failure. */
        PDP_FAIL_ACCESS_CONTROL_LIST_CHECK_FAILURE          = 0x850,    /* Access control list check failure at the lower layer. */
        PDP_FAIL_SERVICE_NOT_ALLOWED_ON_PLMN                = 0x851,    /* Service is not allowed on the requested PLMN. */
        PDP_FAIL_EMM_T3417_EXPIRED                          = 0x852,    /* T3417 timer expiration of the service request procedure. */
        PDP_FAIL_EMM_T3417_EXT_EXPIRED                      = 0x853,    /* Extended service request fails due to expiration of the T3417 EXT timer. */
        PDP_FAIL_RRC_UPLINK_DATA_TRANSMISSION_FAILURE       = 0x854,    /* Transmission failure of radio resource control (RRC) uplink data. */
        PDP_FAIL_RRC_UPLINK_DELIVERY_FAILED_DUE_TO_HANDOVER = 0x855,    /* Radio resource control (RRC) uplink data delivery failed due to a handover. */
        PDP_FAIL_RRC_UPLINK_CONNECTION_RELEASE              = 0x856,    /* Radio resource control (RRC) uplink data delivery failed due to a connection release. */
        PDP_FAIL_RRC_UPLINK_RADIO_LINK_FAILURE              = 0x857,    /* Radio resource control (RRC) uplink data delivery failed due to a radio link failure. */
        PDP_FAIL_RRC_UPLINK_ERROR_REQUEST_FROM_NAS          = 0x858,    /* Radio resource control (RRC) is not connected but the non-access stratum (NAS) sends an uplink data request. */
        PDP_FAIL_RRC_CONNECTION_ACCESS_STRATUM_FAILURE      = 0x859,    /* Radio resource control (RRC) connection failure at access stratum. */
        PDP_FAIL_RRC_CONNECTION_ANOTHER_PROCEDURE_IN_PROGRESS = 0x85A,  /* Radio resource control (RRC) connection establishment is aborted due to another procedure. */
        PDP_FAIL_RRC_CONNECTION_ACCESS_BARRED               = 0x85B,    /* Radio resource control (RRC) connection establishment failed due to access barrred. */
        PDP_FAIL_RRC_CONNECTION_CELL_RESELECTION            = 0x85C,    /* Radio resource control (RRC) connection establishment failed due to cell reselection at access stratum. */
        PDP_FAIL_RRC_CONNECTION_CONFIG_FAILURE              = 0x85D,    /* Connection establishment failed due to configuration failure at the radio resource control (RRC). */
        PDP_FAIL_RRC_CONNECTION_TIMER_EXPIRED               = 0x85E,    /* Radio resource control (RRC) connection could not be established in the time limit. */
        PDP_FAIL_RRC_CONNECTION_LINK_FAILURE                = 0x85F,    /* Connection establishment failed due to a link failure at the radio resource control (RRC). */
        PDP_FAIL_RRC_CONNECTION_CELL_NOT_CAMPED             = 0x860,    /* Connection establishment failed as the radio resource control (RRC) is not camped on any cell. */
        PDP_FAIL_RRC_CONNECTION_SYSTEM_INTERVAL_FAILURE     = 0x861,    /* Connection establishment failed due to a service interval failure at the radio resource control (RRC). */
        PDP_FAIL_RRC_CONNECTION_REJECT_BY_NETWORK           = 0x862,    /* Radio resource control (RRC) connection establishment failed due to the network rejecting the UE connection request. */
        PDP_FAIL_RRC_CONNECTION_NORMAL_RELEASE              = 0x863,    /* Normal radio resource control (RRC) connection release. */
        PDP_FAIL_RRC_CONNECTION_RADIO_LINK_FAILURE          = 0x864,    /* Radio resource control (RRC) connection release failed due to radio link failure conditions. */
        PDP_FAIL_RRC_CONNECTION_REESTABLISHMENT_FAILURE     = 0x865,    /* Radio resource control (RRC) connection re-establishment failure. */
        PDP_FAIL_RRC_CONNECTION_OUT_OF_SERVICE_DURING_CELL_REGISTER = 0x866,    /* UE is out of service during the call register. */
        PDP_FAIL_RRC_CONNECTION_ABORT_REQUEST               = 0x867,    /* Connection has been released by the radio resource control (RRC) due to an abort request. */
        PDP_FAIL_RRC_CONNECTION_SYSTEM_INFORMATION_BLOCK_READ_ERROR = 0x868,    /* Radio resource control (RRC) connection released due to a system information block read error. */
        PDP_FAIL_NETWORK_INITIATED_DETACH_WITH_AUTO_REATTACH = 0x869,    /* Network-initiated detach with reattach. */
        PDP_FAIL_NETWORK_INITIATED_DETACH_NO_AUTO_REATTACH  = 0x86A,    /* Network-initiated detach without reattach. */
        PDP_FAIL_ESM_PROCEDURE_TIME_OUT                     = 0x86B,    /* ESM procedure maximum attempt timeout failure. */
        PDP_FAIL_INVALID_CONNECTION_ID                      = 0x86C,    /* No PDP exists with the given connection ID while modifying or deactivating or activation for an already active PDP. */
        PDP_FAIL_MAXIMIUM_NSAPIS_EXCEEDED                   = 0x86D,    /* Maximum NSAPIs have been exceeded during PDP activation. */
        PDP_FAIL_INVALID_PRIMARY_NSAPI                      = 0x86E,    /* Primary context for NSAPI does not exist. */
        PDP_FAIL_CANNOT_ENCODE_OTA_MESSAGE                  = 0x86F,    /* Unable to encode the OTA message for MT PDP or deactivate PDP. */
        PDP_FAIL_RADIO_ACCESS_BEARER_SETUP_FAILURE          = 0x870,    /* Radio access bearer is not established by the lower layers during activation, modification, or deactivation. */
        PDP_FAIL_PDP_ESTABLISH_TIMEOUT_EXPIRED              = 0x871,    /* Expiration of the PDP establish timer with a maximum of five retries. */
        PDP_FAIL_PDP_MODIFY_TIMEOUT_EXPIRED                 = 0x872,    /* Expiration of the PDP modify timer with a maximum of four retries. */
        PDP_FAIL_PDP_INACTIVE_TIMEOUT_EXPIRED               = 0x873,    /* Expiration of the PDP deactivate timer with a maximum of four retries. */
        PDP_FAIL_PDP_LOWERLAYER_ERROR                       = 0x874,    /* PDP activation failed due to RRC_ABORT or a forbidden PLMN. */
        PDP_FAIL_PDP_MODIFY_COLLISION                       = 0x875,    /* MO PDP modify collision when the MT PDP is already in progress. */
        PDP_FAIL_MAXINUM_SIZE_OF_L2_MESSAGE_EXCEEDED        = 0x876,    /* Maximum size of the L2 message was exceeded. */
        PDP_FAIL_NAS_REQUEST_REJECTED_BY_NETWORK            = 0x877,    /* Non-access stratum (NAS) request was rejected by the network. */
        PDP_FAIL_RRC_CONNECTION_INVALID_REQUEST             = 0x878,    /* Radio resource control (RRC) connection establishment failure due to an error in the request message. */
        PDP_FAIL_RRC_CONNECTION_TRACKING_AREA_ID_CHANGED    = 0x879,    /* Radio resource control (RRC) connection establishment failure due to a change in the tracking area ID. */
        PDP_FAIL_RRC_CONNECTION_RF_UNAVAILABLE              = 0x87A,    /* Radio resource control (RRC) connection establishment failure due to the RF was unavailable. */
        PDP_FAIL_RRC_CONNECTION_ABORTED_DUE_TO_IRAT_CHANGE  = 0x87B,    /* Radio resource control (RRC) connection was aborted before deactivating the LTE stack due to
                                                                         * a successful LTE to WCDMA/GSM/TD-SCDMA IRAT change. */
        PDP_FAIL_RRC_CONNECTION_RELEASED_SECURITY_NOT_ACTIVE = 0x87C,   /* If the UE has an LTE radio link failure before security is established, the radio resource
                                                                         * control (RRC) connection must be released and the UE must return to idle. */
        PDP_FAIL_RRC_CONNECTION_ABORTED_AFTER_HANDOVER      = 0x87D,    /* Radio resource control (RRC) connection was aborted by the non-access stratum (NAS) after an IRAT to LTE IRAT handover. */
        PDP_FAIL_RRC_CONNECTION_ABORTED_AFTER_IRAT_CELL_CHANGE = 0x87E, /* Radio resource control (RRC) connection was aborted before deactivating the LTE stack after a
                                                                         * successful LTE to GSM/EDGE IRAT cell change order procedure. */
        PDP_FAIL_RRC_CONNECTION_ABORTED_DURING_IRAT_CELL_CHANGE = 0x87F,/* Radio resource control (RRC) connection was aborted in the middle of a LTE to GSM IRAT cell change order procedure. */
        PDP_FAIL_IMSI_UNKNOWN_IN_HOME_SUBSCRIBER_SERVER     = 0x880,    /* IMSI present in the UE is unknown in the home subscriber server. */
        PDP_FAIL_IMEI_NOT_ACCEPTED                          = 0x881,    /* IMEI of the UE is not accepted by the network. */
        PDP_FAIL_EPS_SERVICES_AND_NON_EPS_SERVICES_NOT_ALLOWED = 0x882, /* EPS and non-EPS services are not allowed by the network. */
        PDP_FAIL_EPS_SERVICES_NOT_ALLOWED_IN_PLMN           = 0x883,    /* EPS services are not allowed in the PLMN. */
        PDP_FAIL_MSC_TEMPORARILY_NOT_REACHABLE              = 0x884,    /* Mobile switching center is temporarily unreachable. */
        PDP_FAIL_CS_DOMAIN_NOT_AVAILABLE                    = 0x885,    /* CS domain is not available. */
        PDP_FAIL_ESM_FAILURE                                = 0x886,    /* ESM level failure. */
        PDP_FAIL_MAC_FAILURE                                = 0x887,    /* MAC level failure. */
        PDP_FAIL_SYNCHRONIZATION_FAILURE                    = 0x888,    /* Synchronization failure. */
        PDP_FAIL_UE_SECURITY_CAPABILITIES_MISMATCH          = 0x889,    /* UE security capabilities mismatch. */
        PDP_FAIL_SECURITY_MODE_REJECTED                     = 0x88A,    /* Unspecified security mode reject. */
        PDP_FAIL_UNACCEPTABLE_NON_EPS_AUTHENTICATION        = 0x88B,    /* Unacceptable non-EPS authentication. */
        PDP_FAIL_CS_FALLBACK_CALL_ESTABLISHMENT_NOT_ALLOWED = 0x88C,    /* CS fallback call establishment is not allowed. */
        PDP_FAIL_NO_EPS_BEARER_CONTEXT_ACTIVATED            = 0x88D,    /* No EPS bearer context was activated. */
        PDP_FAIL_INVALID_EMM_STATE                          = 0x88E,    /* Invalid EMM state. */
        PDP_FAIL_NAS_LAYER_FAILURE                          = 0x88F,    /* Non-Access Spectrum layer failure. */
        PDP_FAIL_MULTIPLE_PDP_CALL_NOT_ALLOWED              = 0x890,    /* Multiple PDP call feature is disabled. */
        PDP_FAIL_EMBMS_NOT_ENABLED                          = 0x891,    /* Data call has been brought down because EMBMS is not enabled at the RRC layer. */
        PDP_FAIL_IRAT_HANDOVER_FAILED                       = 0x892,    /* Data call was unsuccessfully transferred during the IRAT handover. */
        PDP_FAIL_EMBMS_REGULAR_DEACTIVATION                 = 0x893,    /* EMBMS data call has been successfully brought down. */
        PDP_FAIL_TEST_LOOPBACK_REGULAR_DEACTIVATION         = 0x894,    /* Test loop-back data call has been successfully brought down. */
        PDP_FAIL_LOWER_LAYER_REGISTRATION_FAILURE           = 0x895,    /* Lower layer registration failure. */
        PDP_FAIL_DATA_PLAN_EXPIRED                          = 0x896,    /* Network initiates a detach on LTE with error cause "data plan has been replenished or has expired". */
        PDP_FAIL_UMTS_HANDOVER_TO_IWLAN                     = 0x897,    /* UMTS interface is brought down due to handover from UMTS to iWLAN. */
        PDP_FAIL_EVDO_CONNECTION_DENY_BY_GENERAL_OR_NETWORK_BUSY = 0x898,    /* Received a connection deny due to general or network busy on EVDO network. */
        PDP_FAIL_EVDO_CONNECTION_DENY_BY_BILLING_OR_AUTHENTICATION_FAILURE = 0x899,    /* Received a connection deny due to billing or authentication failure on EVDO network. */
        PDP_FAIL_EVDO_HDR_CHANGED                           = 0x89A,    /* HDR system has been changed due to redirection or the PRL was not preferred. */
        PDP_FAIL_EVDO_HDR_EXITED                            = 0x89B,    /* Device exited HDR due to redirection or the PRL was not preferred. */
        PDP_FAIL_EVDO_HDR_NO_SESSION                        = 0x89C,    /* Device does not have an HDR session. */
        PDP_FAIL_EVDO_USING_GPS_FIX_INSTEAD_OF_HDR_CALL     = 0x89D,    /* It is ending an HDR call origination in favor of a GPS fix. */
        PDP_FAIL_EVDO_HDR_CONNECTION_SETUP_TIMEOUT          = 0x89E,    /* Connection setup on the HDR system was time out. */
        PDP_FAIL_FAILED_TO_ACQUIRE_COLOCATED_HDR            = 0x89F,    /* Device failed to acquire a co-located HDR for origination. */
        PDP_FAIL_OTASP_COMMIT_IN_PROGRESS                   = 0x8A0,    /* OTASP commit is in progress. */
        PDP_FAIL_NO_HYBRID_HDR_SERVICE                      = 0x8A1,    /* Device has no hybrid HDR service. */
        PDP_FAIL_HDR_NO_LOCK_GRANTED                        = 0x8A2,    /* HDR module could not be obtained because of the RF locked. */
        PDP_FAIL_DBM_OR_SMS_IN_PROGRESS                     = 0x8A3,    /* DBM or SMS is in progress. */
        PDP_FAIL_HDR_FADE                                   = 0x8A4,    /* HDR module released the call due to fade. */
        PDP_FAIL_HDR_ACCESS_FAILURE                         = 0x8A5,    /* HDR system access failure. */
        PDP_FAIL_UNSUPPORTED_1X_PREV                        = 0x8A6,    /* P_rev supported by 1 base station is less than 6, which is not supported for a 1X data call.
                                                                         * The UE must be in the footprint of BS which has p_rev >= 6 to support this SO33 call. */
        PDP_FAIL_LOCAL_END                                  = 0x8A7,    /* Client ended the data call. */
        PDP_FAIL_NO_SERVICE                                 = 0x8A8,    /* Device has no service. */
        PDP_FAIL_FADE                                       = 0x8A9,    /* Device lost the system due to fade. */
        PDP_FAIL_NORMAL_RELEASE                             = 0x8AA,    /* Receiving a release from the base station with no reason. */
        PDP_FAIL_ACCESS_ATTEMPT_ALREADY_IN_PROGRESS         = 0x8AB,    /* Access attempt is already in progress. */
        PDP_FAIL_REDIRECTION_OR_HANDOFF_IN_PROGRESS         = 0x8AC,    /* Device is in the process of redirecting or handing off to a different target system. */
        PDP_FAIL_EMERGENCY_MODE                             = 0x8AD,    /* Device is operating in Emergency mode. */
        PDP_FAIL_PHONE_IN_USE                               = 0x8AE,    /* Device is in use (e.g., voice call). */
        PDP_FAIL_INVALID_MODE                               = 0x8AF,    /* Device operational mode is different from the mode requested in the traffic channel bring up. */
        PDP_FAIL_INVALID_SIM_STATE                          = 0x8B0,    /* SIM was marked by the network as invalid for the circuit and/or packet service domain. */
        PDP_FAIL_NO_COLLOCATED_HDR                          = 0x8B1,    /* There is no co-located HDR. */
        PDP_FAIL_UE_IS_ENTERING_POWERSAVE_MODE              = 0x8B2,    /* UE is entering power save mode. */
        PDP_FAIL_DUAL_SWITCH                                = 0x8B3,    /* Dual switch from single standby to dual standby is in progress. */
        PDP_FAIL_PPP_TIMEOUT                                = 0x8B4,    /* Data call bring up fails in the PPP setup due to a timeout. (e.g., an LCP conf ack was not received from the network) */
        PDP_FAIL_PPP_AUTH_FAILURE                           = 0x8B5,    /* Data call bring up fails in the PPP setup due to an authorization failure.
                                                                         * (e.g., authorization is required, but not negotiated with the network during an LCP phase) */
        PDP_FAIL_PPP_OPTION_MISMATCH                        = 0x8B6,    /* Data call bring up fails in the PPP setup due to an option mismatch. */
        PDP_FAIL_PPP_PAP_FAILURE                            = 0x8B7,    /* Data call bring up fails in the PPP setup due to a PAP failure. */
        PDP_FAIL_PPP_CHAP_FAILURE                           = 0x8B8,    /* Data call bring up fails in the PPP setup due to a CHAP failure. */
        PDP_FAIL_PPP_CLOSE_IN_PROGRESS                      = 0x8B9,    /* Data call bring up fails in the PPP setup because the PPP is in the process of cleaning the previous PPP session. */
        PDP_FAIL_LIMITED_TO_IPV4                            = 0x8BA,    /* IPv6 interface bring up fails because the network provided only the IPv4 address for the
                                                                         * upcoming PDN permanent client can reattempt a IPv6 call bring up after the IPv4 interface is
                                                                         * also brought down. However, there is no guarantee that the network will provide a IPv6 address. */
        PDP_FAIL_LIMITED_TO_IPV6                            = 0x8BB,    /* IPv4 interface bring up fails because the network provided only the IPv6 address for the
                                                                         * upcoming PDN permanent client can reattempt a IPv4 call bring up after the IPv6 interface is
                                                                         * also brought down. However there is no guarantee that the network will provide a IPv4 address. */
        PDP_FAIL_VSNCP_TIMEOUT                              = 0x8BC,    /* Data call bring up fails in the VSNCP phase due to a VSNCP timeout error. */
        PDP_FAIL_VSNCP_GEN_ERROR                            = 0x8BD,    /* Data call bring up fails in the VSNCP phase due to a general error. It's used when there is
                                                                         * no other specific error code available to report the failure. */
        PDP_FAIL_VSNCP_APN_UNATHORIZED                      = 0x8BE,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request because the requested APN is unauthorized. */
        PDP_FAIL_VSNCP_PDN_LIMIT_EXCEEDED                   = 0x8BF,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request because the PDN limit has been exceeded. */
        PDP_FAIL_VSNCP_NO_PDN_GATEWAY_ADDRESS               = 0x8C0,    /* Data call bring up fails in the VSNCP phase due to the network rejected the VSNCP
                                                                         * configuration request due to no PDN gateway address. */
        PDP_FAIL_VSNCP_PDN_GATEWAY_UNREACHABLE              = 0x8C1,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request because the PDN gateway is unreachable. */
        PDP_FAIL_VSNCP_PDN_GATEWAY_REJECT                   = 0x8C2,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request due to a PDN gateway reject. */
        PDP_FAIL_VSNCP_INSUFFICIENT_PARAMETERS              = 0x8C3,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request with the reason of insufficient parameter. */
        PDP_FAIL_VSNCP_RESOURCE_UNAVAILABLE                 = 0x8C4,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request with the reason of resource unavailable. */
        PDP_FAIL_VSNCP_ADMINISTRATIVELY_PROHIBITED          = 0x8C5,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request with the reason of administratively prohibited at the HSGW. */
        PDP_FAIL_VSNCP_PDN_ID_IN_USE                        = 0x8C6,    /* Data call bring up fails in the VSNCP phase due to a network rejection of PDN ID in use, or
        ...
        PDP_FAIL_VSNCP_SUBSCRIBER_LIMITATION                = 0x8C7,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request for the reason of subscriber limitation. */
        PDP_FAIL_VSNCP_PDN_EXISTS_FOR_THIS_APN              = 0x8C8,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         * configuration request because the PDN exists for this APN. */
        PDP_FAIL_VSNCP_RECONNECT_NOT_ALLOWED                = 0x8C9,    /* Data call bring up fails in the VSNCP phase due to a network rejection of the VSNCP
                                                                         ...
                                                                         * this error code from the network, the modem infinitely throttles the PDN until the next power cycle. */
        PDP_FAIL_IPV6_PREFIX_UNAVAILABLE                    = 0x8CA,    /* Device failure to obtain the prefix from the network. */
        PDP_FAIL_HANDOFF_PREFERENCE_CHANGED                 = 0x8CB,    /* System preference change back to SRAT during handoff */
...
typedef struct {
    int rsrp;       /* The current Reference Signal Receive Power in dBm multipled by -1. */
    int txSpeed;    /* Lte transmission speed */
    int rxSpeed;    /* Lte receiving speed */
    int snr;        /* Signal to Noise Ratio */
} RIL_LTE_SpeedAndSignalStrength;
...
typedef struct {
    char modemUuid[UUID_MAX_LENGTH];
...
#define MAX_SS_INFO 4
#define SERVICE_CLASSES_NUM 7
...
typedef struct {
    ...
    RIL_CallForwardInfo cfInfo[SERVICE_CLASSES_NUM]; /* This is the response data
...
typedef struct {
    ...
    union {
        int ssInfo[MAX_SS_INFO]; /* This is the response data for most of the SS GET/SET
...
typedef enum {
    RIL_APN_AUTH_TYPE_NO_PAP_NO_CHAP,   // PAP and CHAP is never performed.
    RIL_APN_AUTH_TYPE_PAP_NO_CHAP,      // PAP may be performed; CHAP is never performed.
    RIL_APN_AUTH_TYPE_NO_PAP_CHAP,      // CHAP may be performed; PAP is never performed.
    RIL_APN_AUTH_TYPE_PAP_CHAP          // PAP / CHAP may be performed - baseband dependent.
} RIL_ApnAuthType;

typedef enum {
    RIL_DATA_PROFILE_INFO_TYPE_COMMON,
    RIL_DATA_PROFILE_INFO_TYPE_THREE_GPP,
    RIL_DATA_PROFILE_INFO_TYPE_THREE_GPP2
} RIL_DataProfileInfoType;

typedef struct {
    /* id of the data profile */
    RIL_DataProfile profileId;
    /* the APN to connect to */
    char* apn;
    /** one of the PDP_type values in TS 27.007 section 10.1.1.
     * For example, "IP", "IPV6", "IPV4V6", or "PPP".
...
typedef enum {
    RADIO_EXT_ACCESS_UNKNOWN = 0,      /* Unknown access network */
    RADIO_EXT_ACCESS_NET_GERAN = 1,    /* GSM EDGE Radio Access Network */
    RADIO_EXT_ACCESS_NET_UTRAN = 2,    /* Universal Terrestrial Radio Access Network */
    RADIO_EXT_ACCESS_NET_EUTRAN = 3,   /* Evolved Universal Terrestrial Radio Access Network */
    RADIO_EXT_ACCESS_NET_CDMA2000 = 4, /* CDMA 2000 network */
    RADIO_EXT_ACCESS_NET_IWLAN = 5,    /* Interworking Wireless LAN */
    RADIO_EXT_ACCESS_NET_NGRAN = 6,    /* Next-Generation Radio Access Network */
} RIL_Ext_RadioAccessNetworks;
...
typedef struct {
    ...
    RIL_Ext_RadioAccessNetworks accessNetwork;
} RIL_Ext_SignalStrengthReportingCriteria;

/*********************Structs and Enums Extended in 1.2************************/
/**
 * value are in seconds
 */
typedef enum {
    SCAN_INITERVAL_MIN = 5,
    SCAN_INITERVAL_MAX = 300,
 ...
 */
typedef enum {
    MAX_SERACH_TIME_MIN = 60,
    MAX_SERACH_TIME_MAX = 3600,
 ...
 */
typedef enum {
    INCREMENT_RESULTS_PERIOD_MIN = 1,
    INCREMENT_RESULTS_PERIOD_MAX = 10,
...
/**
 * Overwritten from @1.0::IndicationFilter (RIL_UnsolicitedResponseFilter) in order to redefine ALL.
 * In the future, this should be extended instead of overwritten.
 */
typedef enum {
    IND_FILTER_NONE = 0,
    IND_FILTER_ALL = ~0,
     ...
     */
    IND_FILTER_SIGNAL_STRENGTH = 1 << 0,
    /**
     ...
     */
    IND_FILTER_FULL_NETWORK_STATE = 1 << 1,
    /**
     ...
     */
    IND_FILTER_DATA_CALL_DORMANCY_CHANGED = 1 << 2,
    /**
     ...
     */
    IND_FILTER_LINK_CAPACITY_ESTIMATE = 1 << 3,
    /**
     ...
     */
    IND_FILTER_PHYSICAL_CHANNEL_CONFIG = 1 << 4,
 ...
 */
typedef enum {
    ADUIO_QUALITY_UNSPECIFIED,  /* Unspecified audio codec */
    ADUIO_QUALITY_AMR,          /* AMR (Narrowband) audio codec */
    ADUIO_QUALITY_AMR_WB,       /* AMR (Wideband) audio codec */
    ADUIO_QUALITY_GSM_EFR,      /* GSM Enhanced Full-Rate audio codec */
    ADUIO_QUALITY_GSM_FR,       /* GSM Full-Rate audio codec */
    ADUIO_QUALITY_GSM_HR,       /* GSM Half-Rate audio codec */
    ADUIO_QUALITY_EVRC,         /* Enhanced Variable rate codec */
    ADUIO_QUALITY_EVRC_B,       /* Enhanced Variable rate codec revision B */
    ADUIO_QUALITY_EVRC_WB,      /* Enhanced Variable rate codec (Wideband) */
    ADUIO_QUALITY_EVRC_NW,      /* Enhanced Variable rate codec (Narrowband) */
} RIL_AudioQuality;
...
typedef struct {
     ...
     * RadioConst:RIL_RADIO_ACCESS_SPECIFIER_MAX_SIZE
     */
    RIL_RadioAccessSpecifier specifiers[RIL_RADIO_ACCESS_SPECIFIER_MAX_SIZE];
...
typedef struct {
    char *alphaLong;  // Long alpha Operator Name String or Enhanced Operator Name String.
    char *alphaShort;  // Short alpha Operator Name String or Enhanced Operator Name String
...
} RIL_WcdmaSignalStrength_v1_2;

typedef struct {
    uint32_t signalStrength;  /* UTRA carrier RSSI as defined in TS 25.225 5.1.4
                                 Valid values are (0-31, 99) as defined in TS 27.007 8.5 */
    uint32_t bitErrorRate;    /* Transport Channel BER as defined in TS 25.225 5.2.5
                                 Valid values are (0-7, 99) as defined in TS 27.007 8.5 */
    uint32_t rscp;            /* P-CCPCH RSCP as defined in TS 25.225 5.1.1
                                 Valid values are (0-96, 255) as defined in TS 27.007 8.69 */
} RIL_TdscdmaSignalStrength_v1_2;
...
typedef struct {
    ...
    RIL_WcdmaSignalStrength_v1_2 signalStrengthWcdma;
...
typedef struct {
    ...
    RIL_TdscdmaSignalStrength_v1_2 signalStrengthTdscdma;
...
typedef struct {
    RIL_ScanStatus status;          // The status of the scan.
    uint32_t network_infos_length;  // Total length of RIL_CellInfo
    RIL_CellInfo_v1_2 *network_infos;// List of network information as CellInfo.
    RIL_Errno error;                // The error code of the incremental result.
} RIL_NetworkScanResult_v1_2;

typedef struct {
    RIL_GSM_SignalStrength_v12 gsm;
    RIL_CDMA_SignalStrength cdma;
    RIL_EVDO_SignalStrength evdo;
    RIL_LTE_SignalStrength_v8 lte;
    RIL_TdscdmaSignalStrength_v1_2 tdScdma;
    RIL_WcdmaSignalStrength_v1_2 wcdma;
...
typedef struct {
    RIL_CellConnectionStatus status;  // Connection status for cell.
                                      // Valid values are PRIMARY_SERVING and SECONDARY_SERVING.
    int32_t cellBandwidthDownlink;    // Cell bandwidth, in kHz.
...
typedef struct {
    ...
    RIL_CellIdentity_v1_2 cellIdentity;
} RIL_VoiceRegStateResult_v1_2;  // RIL_VoiceRegistrationStateResponse
...
typedef struct {
    ...
    RIL_CellIdentity_v1_2 cellIdentity;
} RIL_DataRegStateResult_v1_2;  // RIL_DataRegistrationStateResponse
...
typedef struct {
    ...
    int32_t thresholdsDbmNumber;
    int32_t *thresholdsDbm;
    RIL_RadioAccessNetworks_v1_2 accessNetwork;
...
/*********************Structs and Enums Extended in 1.3************************/
typedef enum {
    RESPONSE_TYPE_SOLICITED,
    RESPONSE_TYPE_SOLICITED_ACK,
    RESPONSE_TYPE_SOLICITED_ACK_EXP,
} RIL_RadioResponseType;

typedef struct {
    RIL_RadioResponseType   type;       // Response type
    int32_t                 serial;     // Serial number of the request
    RIL_Errno               error;      // Response error
    bool                    isEnabled;  // whether modem is enabled or not
} RIL_RadioResponseInfoModem;

typedef struct {
    bool                        specifyChannels;
    uint32_t                    specifiers_length;  // Length of specifiers
    RIL_RadioAccessSpecifier    specifiers[RIL_RADIO_ACCESS_SPECIFIER_MAX_SIZE];  // Radio access networks with bands/channels.
 ...
 */
typedef struct {
    char *      number;     /* The emergency number. The character in the number string should only
                              * be the dial pad character('0'-'9', '*', or '#'). For example: 911. */
 ...
 */
typedef enum {
    FREQUENCY_RANGE_LOW = 1,    /* Indicates the frequency range is below 1GHz. */
    FREQUENCY_RANGE_MID = 2,    /* Indicates the frequency range is between 1GHz and 3GHz. */
    FREQUENCY_RANGE_HIGH = 3,   /* Indicates the frequency range is between 3GHz and 6GHz. */
    FREQUENCY_RANGE_MMWAVE = 4, /* Indicates the frequency range is above 6GHz (millimeter wave frequency). */
} RIL_FrequencyRange;
 ...
 */
typedef enum {
    DATA_CONN_STATUS_INACTIVE   = 0,  /* Indicates the data connection is inactive. */
    DATA_CONN_STATUS_DORMANT    = 1,  /* Indicates the data connection is active with physical link dormant. */
    DATA_CONN_STATUS_ACTIVE     = 2,  /* Indicates the data connection is active with physical link up. */
 ...
 */
typedef enum {
    PDP_PROTOCOL_TYPE_UNKNOWN       = -1,   /* Unknown protocol */
    PDP_PROTOCOL_TYPE_IP            = 0,    /* Internet protocol */
    PDP_PROTOCOL_TYPE_IPV6          = 1,    /* Internet protocol, version 6 */
    PDP_PROTOCOL_TYPE_IPV4V6        = 2,    /* Virtual PDP type introduced to handle dual IP stack UE capability. */
    PDP_PROTOCOL_TYPE_PPP           = 3,    /* Point to point protocol */
    PDP_PROTOCOL_TYPE_NON_IP        = 4,    /* Transfer of Non-IP data to external packet data network */
    PDP_PROTOCOL_TYPE_UNSTRUCTURED  = 5,    /* Transfer of Unstructured data to the Data Network via N6 */
} RIL_PdpProtocolType;

typedef union {
    RIL_FrequencyRange  range;          /* A rough frequency range. */
    int32_t             channelNumber;  /* The Absolute Radio Frequency Channel Number. */
} RIL_RadioFrequencyInfo;

typedef struct {
    RIL_PhysicalChannelConfig   base;           /* see @1.2::PhysicalChannelConfig */
    RIL_RadioTechnology         rat;            /* The radio technology for this physical channel. */
    RIL_RadioFrequencyInfo      rfInfo;         /* The radio frequency info. */
    int32_t *                   contextIds;     /* A list of data calls mapped to this physical channel. The context id must match the cid of
                                                 * @1.4::SetupDataCallResult. An empty list means the physical channel has no data call mapped to it. */
    int                         contextIdsLen;
    uint32_t                    physicalCellId; /* The physical cell identifier for this cell.
                                                 ...
                                                 * In 5G RAN, this value is physical layer cell identity. The range is [0, 1008]. Reference: 3GPP TS 38.211 section 7.4.2.1. */
 ...
 */
typedef struct {
    bool isVopsSupported;       /* This indicates if camped network support VoLTE services. This information is received
    ...
    bool isEmcBearerSupported;  /* This indicates if camped network support VoLTE emergency bearers. This information
...
typedef struct {
    bool isEndcAvailable;    /* Indicates that if E-UTRA-NR Dual Connectivity (EN-DC) is supported by the primary serving cell.
    ...
    bool isDcNrRestricted;   /* True if use of dual connectivity with NR is restricted. Reference: 3GPP TS 24.301 v15.03 section 9.3.3.12A. */
    bool isNrAvailable;      /* True if the bit N is in the PLMN-InfoList-r15 is true and the selected PLMN is present in
...
typedef struct  {
    RIL_DataRegStateResult_v1_2 base;  /* see @1.2::DataRegStateResult */
    /**
    ...
    union VopsInfo {
        uint32_t noinit;
        RIL_LteVopsInfo lteVopsInfo;  /* LTE network capability */
    } vopsInfo;

    RIL_NrIndicators nrIndicators;    /* The parameters of NR 5G Non-Standalone. This value is only
                                       * valid on E-UTRAN, otherwise must be empty. */
} RIL_DataRegStateResult_v1_4;

/** Contains the configuration of the LTE cell tower. */
typedef struct  {
    bool isEndcAvailable;   /* Indicates that if E-UTRA-NR Dual Connectivity (EN-DC) is supported by the LTE cell.
...
typedef struct  {
    RIL_CellInfoLte_v1_2    base;
    RIL_CellConfigLte       cellConfig;
} RIL_CellInfoLte_v1_4;

typedef struct {
    int32_t     ssRsrp;    /* SS reference signal received power, multipled by -1. Reference: 3GPP TS 38.215.
                            * Range [44, 140], INT_MAX means invalid/unreported. */
    int32_t     ssRsrq;    /* SS reference signal received quality, multipled by -1.  Reference: 3GPP TS 38.215.
                            * Range [3, 20], INT_MAX means invalid/unreported.*/
    int32_t     ssSinr;    /* SS signal-to-noise and interference ratio. Reference: 3GPP TS 38.215 section 5.1.*, 3GPP TS 38.133 section 10.1.16.1.
                            * Range [-23, 40], INT_MAX means invalid/unreported. */
    int32_t     csiRsrp;   /* CSI reference signal received power, multipled by -1. Reference: 3GPP TS 38.215.
                            * Range [44, 140], INT_MAX means invalid/unreported. */
    int32_t     csiRsrq;   /* CSI reference signal received quality, multipled by -1. Reference: 3GPP TS 38.215.
                            * Range [3, 20], INT_MAX means invalid/unreported. */
    int32_t     csiSinr;   /* CSI signal-to-noise and interference ratio. Reference: 3GPP TS 138.215 section 5.1.*, 3GPP TS 38.133 section 10.1.16.1.
                            * Range [-23, 40], INT_MAX means invalid/unreported. */
} RIL_NrSignalStrength;

typedef struct {
    int         mcc;    /* 3-digit Mobile Country Code, in range[0, 999]; This value must be valid for registered or
    ...
    int32_t     nrarfcn;/* NR Absolute Radio Frequency Channel Number, in range [0, 3279165].
                         * Reference: 3GPP TS 38.101-1 and 3GPP TS 38.101-2 section 5.4.2.1. This value must be valid. */
    RIL_CellIdentityOperatorNames operatorNames;
} RIL_CellIdentityNr;
...
typedef struct  {
    bool                     isRegistered;      /* True if the phone is registered to a mobile network that provides service on this cell and
...
typedef struct {
    RIL_ScanStatus      status;                 /* he status of the scan. */
    RIL_Errno           error;                  /* The error code of the incremental result. */
    uint32_t            network_infos_length;   /* Total length of RIL_CellInfo_v1_4 */
    RIL_CellInfo_v1_4 * networkInfos;           /* List of network information as CellInfo. */
 ...
 */
typedef struct {
    RIL_DataProfile         profileId;              /* id of the data profile */
    char *                  apn;                    /* The APN name */
    RIL_PdpProtocolType     protocol;               /* PDP_type values */
    RIL_PdpProtocolType     roamingProtocol;        /* PDP_type values used on roaming network */
    RIL_ApnAuthType         authType;               /* APN authentication type */
    char *                  user;                   /* The username for APN, or empty string */
    char *                  password;               /* The password for APN, or empty string */
    RIL_DataProfileInfoType type;                   /* Data profile technology type */
    ...
    int32_t                 waitTime;               /* The required wait time in seconds after a successful UE initiated disconnect of a given PDN
    ...
    int32_t                 supportedApnTypesBitmap;/* Supported APN types bitmap. See ApnTypes for the value of each bit. bitfield<ApnTypes> */
    int32_t                 bearerBitmap;           /* The bearer bitmap. See RadioAccessFamily for the value of each bit. bitfield<RadioAccessFamily> */
    int32_t                 mtu;                    /* Maximum transmission unit (MTU) size in bytes */
    bool                    preferred;              /* True if this data profile was used to bring up the last default (i.e internet) data connection successfully. */
    bool                    persistent;             /* If true, modem must persist this data profile and profileId must not be
                                                     * set to DataProfileId.INVALID. If the same data profile exists, this data profile must overwrite it. */
...
typedef struct {
    int32_t                 session;            /* Unique session value defined by fr amework returned in all "responses/unslo". */
    RadioCapabilityPhase    phase;
    int                     raf;                /* 32-bit bitmap of RadioAccessFamily. bitfield<RadioAccessFamily>*/
    char *                  logicalModemUuid;   /* A UUID typically "com.xxxx.lmX" where X is the logical modem.
                                                 * RadioConst:MAX_UUID_LENGTH is the max length. */
    RadioCapabilityStatus status;
} RIL_RadioCapability_v1_4;

/**
 * Overwritten from @1.0::SetupDataCallResult (RIL_Data_Call_Response_v11) in order to
 * update the DataCallFailCause to 1.4 version.
 */
typedef struct {
    RIL_DataCallFailCause       cause;              /* Data call fail cause. DataCallFailCause.NONE if no error. */
    int32_t                     suggestedRetryTime; /* If status != DataCallFailCause.NONE, this field indicates the suggested retry back-off timer
    ...
    RIL_DataConnActiveStatus    active;             /* Data connection active status. */
    RIL_PdpProtocolType         type;               /* PDP_type values. If cause is DataCallFailCause.ONLY_SINGLE_BEARER_ALLOWED, this is the type
                                                     * supported such as "IP" or "IPV6". */
    char *                      ifname;             /* The network interface name. */
    uint32_t                    addressesNumber;
    char **                     addresses;          /* List of addresses with optional "/" prefix length, e.g., "192.0.1.3" or
    ...
    uint32_t                    dnsesNumber;
    char **                     dnses;              /* List of DNS server addresses, e.g., "192.0.1.3" or "192.0.1.11 2001:db8::1". Empty if no dns
                                                     * server addresses returned. */
    uint32_t                    gatewaysNumber;
    char **                     gateways;           /* List of default gateway addresses, e.g., "192.0.1.3" or "192.0.1.11 2001:db8::1".
                                                     * When empty, the addresses represent point to point connections. */
    uint32_t                    pcscfNumber;
    char **                     pcscf;              /* List of P-CSCF(Proxy Call State Control Function) addresses via PCO(Protocol Configuration
                                                     * Option), e.g., "2001:db8::1 2001:db8::2 2001:db8::3". Empty if not IMS client. */
    int32_t                     mtu;                /* MTU received from network. Value <= 0 means network has either not sent a value or sent an invalid value. */
} RIL_SetupDataCallResult_v1_4;

typedef enum {
    SIM_POLICY_NO_MULTISIM_POLICY            = 0,    /* Indicates that configuration applies to each slot independently. */
    SIM_POLICY_ONE_VALID_SIM_MUST_BE_PRESENT = 1,    /* Indicates that any SIM card can be used as far as one valid card is present in the device.
...
typedef struct {
    int32_t         allowedCarriersNumber;
    int32_t         excludedCarriersNumber;
    RIL_Carrier *   allowedCarriers;    /* List of allowed carriers. The character '?' is used as wildcard character to match any value. */
    RIL_Carrier *   excludedCarriers;   /* List of excluded carriers. * The character '?' is used as wildcard character to match any value. */
    bool allowedCarriersPrioritized;    /* True means that only carriers included in the allowed list and not in the excluded list
...
typedef struct  {
    RIL_GSM_SignalStrength_v12      gsm;    /* If GSM measurements are provided, this structure must contain valid measurements; otherwise
                                             * all fields should be set to INT_MAX to mark them as invalid. */
    RIL_CDMA_SignalStrength         cdma;   /* If CDMA measurements are provided, this structure must contain valid measurements; otherwise
                                             * all fields should be set to INT_MAX to mark them as invalid. */
    RIL_EVDO_SignalStrength         evdo;   /* If EvDO measurements are provided, this structure must contain valid measurements; otherwise
                                             * all fields should be set to INT_MAX to mark them as invalid. */
    RIL_LTE_SignalStrength_v8       lte;    /* If LTE measurements are provided, this structure must contain valid measurements; otherwise
                                             * all fields should be set to INT_MAX to mark them as invalid. */
    RIL_TdscdmaSignalStrength_v1_2  tdscdma;/* If TD-SCDMA measurements are provided, this structure must contain valid measurements;
                                             * otherwise all fields should be set to INT_MAX to mark them as invalid. */
    RIL_WcdmaSignalStrength_v1_2    wcdma;  /* If WCDMA measurements are provided, this structure must contain valid measurements; otherwise
                                             * all fields should be set to INT_MAX to mark them as invalid. */
    RIL_NrSignalStrength            nr;     /* If NR 5G measurements are provided, this structure must contain valid measurements; otherwise
                                             * all fields should be set to INT_MAX to mark them as invalid. */
} RIL_SignalStrength_v1_4;

typedef struct {
    // RIL_CarrierRestrictionsWithPriority restrictions;
    uint32_t         allowedCarriersNumber;  // Note: the first four members should be the same as RIL_CarrierRestrictions, do not change!!!
    uint32_t         excludedCarriersNumber;
    RIL_Carrier *   allowedCarriers;    /* List of allowed carriers. The character '?' is used as wildcard character to match any value. */
    RIL_Carrier *   excludedCarriers;   /* List of excluded carriers. * The character '?' is used as wildcard character to match any value. */
    bool allowedCarriersPrioritized;    /* True means that only carriers included in the allowed list and not in the excluded list
                                         * are permitted. Eg. allowedCarriers match mcc/mnc, excludedCarriers has same mcc/mnc and
                                         ...
                                         * False means that all carriers are allowed except those included in the excluded list
                                         * and not in the allowed list. */
    RIL_SimLockMultiSimPolicy           simPolicy;
...
typedef bool (*SE_init)(int simId);

typedef void (*SE_getAtr)(int simId, void *response, int *responseLen);

typedef bool (*SE_isCardPresent)(int simId);
...
/* Need to correspond with types.hal @{*/
typedef enum {
    MODEM_STATUS_ALIVE,
    MODEM_STATUS_ASSERT,
    MODEM_STATUS_BLOCKED,
    MODEM_STATUS_RESET,
} RIL_ModemDetailStatus;

typedef struct {
    RIL_ModemDetailStatus modemState;
    char* assertInfo;
} RIL_ModemStatusInfo;
/* @} */
...
typedef enum {
    ...
    EVDO_AND_CDMA = 64,  //1x/EVDO
    CDMA_ONLY = 65,      //1x only
    EVDO_ONLY = 66,      //do only
    TD_LTE_AND_LTE_FDD_AND_TD_AND_WCDMA_AND_EVDO_AND_CDMA_AND_GSM = 73,
    NR_ONLY = 128,
    NR_AND_TD_LTE_AND_LTE_FDD = 131,
    NR_AND_TD_LTE_AND_LTE_FDD_AND_WCDMA_AND_GSM = 134,
    NONE = 254,
    TD_AND_WCDMA_AND_EVDO_AND_CDMA_AND_GSM = 255,
} RadioFeatures;
 ...
 */
#define RIL_REQUEST_SET_SYSTEM_SELECTION_CHANNELS 148
 ...
 */
#define RIL_REQUEST_ENABLE_MODEM 149
 ...
 */
#define RIL_REQUEST_GET_MODEM_STATUS 150
...
#define RIL_REQUEST_GET_PREFERRED_NETWORK_TYPE_BITMAP 152
...
#define RIL_REQUEST_SET_PREFERRED_NETWORK_TYPE_BITMAP 153

#define RIL_REQUEST_LAST    RIL_REQUEST_SET_PREFERRED_NETWORK_TYPE_BITMAP
...
/**
 * RIL_UNSOL_LINK_CAPACITY_ESTIMATE
 *
 ...
 */
#define RIL_UNSOL_LINK_CAPACITY_ESTIMATE 1051
 ...
 */
#define RIL_UNSOL_PHYSICAL_CHANNEL_CONFIG 1052

#define RIL_UNSOL_EMERGENCY_NUMBER_LIST 1053

#define RIL_UNSOL_RESPONSE_LAST RIL_UNSOL_EMERGENCY_NUMBER_LIST

/* Radio Config unsol response @{ */
#define RIL_UNSOL_RESPONSE_RADIO_CONFIG_BASE 1600
 ...
 *        the device
 */
#define RIL_UNSOL_CONFIG_ICC_SLOT_STATUS 1600
...
#define RIL_EXT_REQUEST_TRAFFIC_CLASS                   (RIL_EXT_REQUEST_BASE + 7)
#define RIL_EXT_REQUEST_ENABLE_LTE                      (RIL_EXT_REQUEST_BASE + 8)
#define RIL_EXT_REQUEST_ATTACH_DATA                     (RIL_EXT_REQUEST_BASE + 9)
#define RIL_EXT_REQUEST_FORCE_DETACH                    (RIL_EXT_REQUEST_BASE + 10)
#define RIL_EXT_REQUEST_GET_HD_VOICE_STATE              (RIL_EXT_REQUEST_BASE + 11)
#define RIL_EXT_REQUEST_SIMMGR_SIM_POWER                (RIL_EXT_REQUEST_BASE + 12)
#define RIL_EXT_REQUEST_ENABLE_RAU_NOTIFY               (RIL_EXT_REQUEST_BASE + 13)
#define RIL_EXT_REQUEST_SIM_GET_ATR                     (RIL_EXT_REQUEST_BASE + 14)
#define RIL_EXT_REQUEST_EXPLICIT_CALL_TRANSFER          (RIL_EXT_REQUEST_BASE + 15)
#define RIL_EXT_REQUEST_GET_SIM_CAPACITY                (RIL_EXT_REQUEST_BASE + 16)
#define RIL_EXT_REQUEST_STORE_SMS_TO_SIM                (RIL_EXT_REQUEST_BASE + 17)
#define RIL_EXT_REQUEST_QUERY_SMS_STORAGE_MODE          (RIL_EXT_REQUEST_BASE + 18)
#define RIL_EXT_REQUEST_GET_SIMLOCK_REMAIN_TIMES        (RIL_EXT_REQUEST_BASE + 19)
#define RIL_EXT_REQUEST_SET_FACILITY_LOCK_FOR_USER      (RIL_EXT_REQUEST_BASE + 20)
#define RIL_EXT_REQUEST_GET_SIMLOCK_STATUS              (RIL_EXT_REQUEST_BASE + 21)
#define RIL_EXT_REQUEST_GET_SIMLOCK_DUMMYS              (RIL_EXT_REQUEST_BASE + 22)
#define RIL_EXT_REQUEST_GET_SIMLOCK_WHITE_LIST          (RIL_EXT_REQUEST_BASE + 23)
#define RIL_EXT_REQUEST_UPDATE_ECCLIST                  (RIL_EXT_REQUEST_BASE + 24)
#define RIL_EXT_REQUEST_SET_SINGLE_PDN                  (RIL_EXT_REQUEST_BASE + 25)
#define RIL_EXT_REQUEST_QUERY_COLP                      (RIL_EXT_REQUEST_BASE + 26)
#define RIL_EXT_REQUEST_QUERY_COLR                      (RIL_EXT_REQUEST_BASE + 27)
#define RIL_EXT_REQUEST_UPDATE_OPERATOR_NAME            (RIL_EXT_REQUEST_BASE + 28)
#define RIL_EXT_REQUEST_SIMMGR_GET_SIM_STATUS           (RIL_EXT_REQUEST_BASE + 29)
#define RIL_EXT_REQUEST_SET_XCAP_IP_ADDR                (RIL_EXT_REQUEST_BASE + 30)
#define RIL_EXT_REQUEST_SEND_CMD                        (RIL_EXT_REQUEST_BASE + 31)
#define RIL_EXT_REQUEST_REATTACH                        (RIL_EXT_REQUEST_BASE + 32)
#define RIL_EXT_REQUEST_SET_PREFERRED_NETWORK_TYPE      (RIL_EXT_REQUEST_BASE + 33)
#define RIL_EXT_REQUEST_SHUTDOWN                        (RIL_EXT_REQUEST_BASE + 34)
#define RIL_EXT_REQUEST_UPDATE_CLIP                     (RIL_EXT_REQUEST_BASE + 35)
#define RIL_EXT_REQUEST_SET_TPMR_STATE                  (RIL_EXT_REQUEST_BASE + 36)
#define RIL_EXT_REQUEST_GET_TPMR_STATE                  (RIL_EXT_REQUEST_BASE + 37)
#define RIL_EXT_REQUEST_SET_VIDEO_RESOLUTION            (RIL_EXT_REQUEST_BASE + 38)
#define RIL_EXT_REQUEST_ENABLE_LOCAL_HOLD               (RIL_EXT_REQUEST_BASE + 39)
#define RIL_EXT_REQUEST_ENABLE_WIFI_PARAM_REPORT        (RIL_EXT_REQUEST_BASE + 40)
#define RIL_EXT_REQUEST_CALL_MEDIA_CHANGE_REQUEST_TIMEOUT   (RIL_EXT_REQUEST_BASE + 41)
#define RIL_EXT_REQUEST_SET_LOCAL_TONE                  (RIL_EXT_REQUEST_BASE + 42)
#define RIL_EXT_REQUEST_UPDATE_PLMN                     (RIL_EXT_REQUEST_BASE + 43)
#define RIL_EXT_REQUEST_QUERY_PLMN                      (RIL_EXT_REQUEST_BASE + 44)
#define RIL_EXT_REQUEST_SIM_POWER_REAL                  (RIL_EXT_REQUEST_BASE + 45)
#define RIL_EXT_REQUEST_GET_RADIO_PREFERENCE            (RIL_EXT_REQUEST_BASE + 46)
#define RIL_EXT_REQUEST_SET_RADIO_PREFERENCE            (RIL_EXT_REQUEST_BASE + 47)
#define RIL_EXT_REQUEST_GET_IMS_CURRENT_CALLS           (RIL_EXT_REQUEST_BASE + 48)
#define RIL_EXT_REQUEST_SET_IMS_VOICE_CALL_AVAILABILITY (RIL_EXT_REQUEST_BASE + 49)
#define RIL_EXT_REQUEST_GET_IMS_VOICE_CALL_AVAILABILITY (RIL_EXT_REQUEST_BASE + 50)
#define RIL_EXT_REQUEST_INIT_ISIM                       (RIL_EXT_REQUEST_BASE + 51)
#define RIL_EXT_REQUEST_IMS_CALL_REQUEST_MEDIA_CHANGE   (RIL_EXT_REQUEST_BASE + 52)
#define RIL_EXT_REQUEST_IMS_CALL_RESPONSE_MEDIA_CHANGE  (RIL_EXT_REQUEST_BASE + 53)
#define RIL_EXT_REQUEST_SET_IMS_SMSC                    (RIL_EXT_REQUEST_BASE + 54)
#define RIL_EXT_REQUEST_IMS_CALL_FALL_BACK_TO_VOICE     (RIL_EXT_REQUEST_BASE + 55)
#define RIL_EXT_REQUEST_QUERY_CALL_FORWARD_STATUS_URI   (RIL_EXT_REQUEST_BASE + 56)
#define RIL_EXT_REQUEST_SET_CALL_FORWARD_URI            (RIL_EXT_REQUEST_BASE + 57)
#define RIL_EXT_REQUEST_IMS_INITIAL_GROUP_CALL          (RIL_EXT_REQUEST_BASE + 58)
#define RIL_EXT_REQUEST_IMS_ADD_TO_GROUP_CALL           (RIL_EXT_REQUEST_BASE + 59)
#define RIL_EXT_REQUEST_ENABLE_IMS                      (RIL_EXT_REQUEST_BASE + 60)
#define RIL_EXT_REQUEST_GET_IMS_BEARER_STATE            (RIL_EXT_REQUEST_BASE + 61)
#define RIL_EXT_REQUEST_SET_INITIAL_ATTACH_APN          (RIL_EXT_REQUEST_BASE + 62)
#define RIL_EXT_REQUEST_IMS_HANDOVER                    (RIL_EXT_REQUEST_BASE + 63)
#define RIL_EXT_REQUEST_IMS_HANDOVER_STATUS_UPDATE      (RIL_EXT_REQUEST_BASE + 64)
#define RIL_EXT_REQUEST_IMS_NETWORK_INFO_CHANGE         (RIL_EXT_REQUEST_BASE + 65)
#define RIL_EXT_REQUEST_IMS_HANDOVER_CALL_END           (RIL_EXT_REQUEST_BASE + 66)
#define RIL_EXT_REQUEST_IMS_WIFI_ENABLE                 (RIL_EXT_REQUEST_BASE + 67)
#define RIL_EXT_REQUEST_IMS_WIFI_CALL_STATE_CHANGE      (RIL_EXT_REQUEST_BASE + 68)
#define RIL_EXT_REQUEST_IMS_UPDATE_DATA_ROUTER          (RIL_EXT_REQUEST_BASE + 69)
#define RIL_EXT_REQUEST_IMS_HOLD_SINGLE_CALL            (RIL_EXT_REQUEST_BASE + 70)
#define RIL_EXT_REQUEST_IMS_MUTE_SINGLE_CALL            (RIL_EXT_REQUEST_BASE + 71)
#define RIL_EXT_REQUEST_IMS_SILENCE_SINGLE_CALL         (RIL_EXT_REQUEST_BASE + 72)
#define RIL_EXT_REQUEST_IMS_ENABLE_LOCAL_CONFERENCE     (RIL_EXT_REQUEST_BASE + 73)
#define RIL_EXT_REQUEST_IMS_NOTIFY_HANDOVER_CALL_INFO   (RIL_EXT_REQUEST_BASE + 74)
#define RIL_EXT_REQUEST_GET_IMS_SRVCC_CAPBILITY         (RIL_EXT_REQUEST_BASE + 75)
#define RIL_EXT_REQUEST_GET_IMS_PCSCF_ADDR              (RIL_EXT_REQUEST_BASE + 76)
#define RIL_EXT_REQUEST_SET_IMS_PCSCF_ADDR              (RIL_EXT_REQUEST_BASE + 77)
#define RIL_EXT_REQUEST_QUERY_FACILITY_LOCK             (RIL_EXT_REQUEST_BASE + 78)
#define RIL_EXT_REQUEST_IMS_REGADDR                     (RIL_EXT_REQUEST_BASE + 79)
#define RIL_EXT_REQUEST_GET_PREFERRED_NETWORK_TYPE      (RIL_EXT_REQUEST_BASE + 80)
#define RIL_EXT_REQUEST_RADIO_POWER_FALLBACK            (RIL_EXT_REQUEST_BASE + 81)
#define RIL_EXT_REQUEST_GET_CNAP                        (RIL_EXT_REQUEST_BASE + 82)
#define RIL_EXT_REQUEST_SET_LOCATION_INFO               (RIL_EXT_REQUEST_BASE + 83)
#define RIL_EXT_REQUEST_GET_SPECIAL_RATCAP              (RIL_EXT_REQUEST_BASE + 84)
#define RIL_EXT_REQUEST_GET_VIDEO_RESOLUTION            (RIL_EXT_REQUEST_BASE + 85)
#define RIL_EXT_REQUEST_GET_IMS_PANI_INFO               (RIL_EXT_REQUEST_BASE + 86)
#define RIL_EXT_REQUEST_SET_EMERGENCY_ONLY              (RIL_EXT_REQUEST_BASE + 87)
#define RIL_EXT_REQUEST_GET_SUBSIDYLOCK_STATUS          (RIL_EXT_REQUEST_BASE + 88)
#define RIL_EXT_REQUEST_SET_IMS_USER_AGENT              (RIL_EXT_REQUEST_BASE + 89)
#define RIL_EXT_REQUEST_RESET_MODEM                     (RIL_EXT_REQUEST_BASE + 90)
#define RIL_EXT_REQUEST_GET_VOLTE_ALLOWED_PLMN          (RIL_EXT_REQUEST_BASE + 91)
#define RIL_EXT_REQUEST_SET_SMS_BEARER                  (RIL_EXT_REQUEST_BASE + 92)
#define RIL_EXT_REQUEST_GET_SMS_BEARER                  (RIL_EXT_REQUEST_BASE + 93)
#define RIL_EXT_REQUEST_QUERY_ROOT_NODE                 (RIL_EXT_REQUEST_BASE + 94)
#define RIL_EXT_REQUEST_PS_DATA_OFF                     (RIL_EXT_REQUEST_BASE + 95)
#define RIL_EXT_REQUEST_LTE_SPEED_AND_SIGNAL_STRENGTH   (RIL_EXT_REQUEST_BASE + 96)
#define RIL_EXT_REQUEST_ENABLE_NR_SWITCH                (RIL_EXT_REQUEST_BASE + 97)
#define RIL_EXT_REQUEST_SET_USBSHARE_SWITCH             (RIL_EXT_REQUEST_BASE + 98)
#define RIL_EXT_REQUEST_SET_STAND_ALONE                 (RIL_EXT_REQUEST_BASE + 99)
#define RIL_EXT_REQUEST_GET_STAND_ALONE                 (RIL_EXT_REQUEST_BASE + 100)
#define RIL_EXT_REQUEST_LAST                             RIL_EXT_REQUEST_GET_STAND_ALONE
...
#define RIL_EXT_UNSOL_RAU_SUCCESS                       (RIL_EXT_UNSOL_RESPONSE_BASE + 8)
#define RIL_EXT_UNSOL_CLEAR_CODE_FALLBACK               (RIL_EXT_UNSOL_RESPONSE_BASE + 9)
#define RIL_EXT_UNSOL_RIL_CONNECTED                     (RIL_EXT_UNSOL_RESPONSE_BASE + 10)
#define RIL_EXT_UNSOL_SIMLOCK_SIM_EXPIRED               (RIL_EXT_UNSOL_RESPONSE_BASE + 11)
#define RIL_EXT_UNSOL_SIM_PS_REJECT                     (RIL_EXT_UNSOL_RESPONSE_BASE + 12)
#define RIL_EXT_UNSOL_SIMMGR_SIM_STATUS_CHANGED         (RIL_EXT_UNSOL_RESPONSE_BASE + 13)
#define RIL_EXT_UNSOL_EARLY_MEDIA                       (RIL_EXT_UNSOL_RESPONSE_BASE + 14)
#define RIL_EXT_UNSOL_SPUCOPS_LIST                      (RIL_EXT_UNSOL_RESPONSE_BASE + 15)
#define RIL_EXT_UNSOL_RESPONSE_IMS_CALL_STATE_CHANGED   (RIL_EXT_UNSOL_RESPONSE_BASE + 16)
#define RIL_EXT_UNSOL_RESPONSE_VIDEO_QUALITY            (RIL_EXT_UNSOL_RESPONSE_BASE + 17)
#define RIL_EXT_UNSOL_RESPONSE_IMS_BEARER_ESTABLISTED   (RIL_EXT_UNSOL_RESPONSE_BASE + 18)
#define RIL_EXT_UNSOL_IMS_HANDOVER_REQUEST              (RIL_EXT_UNSOL_RESPONSE_BASE + 19)
#define RIL_EXT_UNSOL_IMS_HANDOVER_STATUS_CHANGE        (RIL_EXT_UNSOL_RESPONSE_BASE + 20)
#define RIL_EXT_UNSOL_IMS_NETWORK_INFO_CHANGE           (RIL_EXT_UNSOL_RESPONSE_BASE + 21)
#define RIL_EXT_UNSOL_IMS_REGISTER_ADDRESS_CHANGE       (RIL_EXT_UNSOL_RESPONSE_BASE + 22)
#define RIL_EXT_UNSOL_IMS_WIFI_PARAM                    (RIL_EXT_UNSOL_RESPONSE_BASE + 23)
#define RIL_EXT_UNSOL_IMS_NETWORK_STATE_CHANGED         (RIL_EXT_UNSOL_RESPONSE_BASE + 24)
#define RIL_EXT_UNSOL_UPDATE_HD_VOICE_STATE             (RIL_EXT_UNSOL_RESPONSE_BASE + 25)
#define RIL_EXT_UNSOL_SUBSIDYLOCK_STATUS_CHANGED        (RIL_EXT_UNSOL_RESPONSE_BASE + 26)
#define RIL_EXT_UNSOL_IMS_CSFB_VENDOR_CAUSE             (RIL_EXT_UNSOL_RESPONSE_BASE + 27)
#define RIL_EXT_UNSOL_IMS_ERROR_CAUSE                   (RIL_EXT_UNSOL_RESPONSE_BASE + 28)
#define RIL_EXT_UNSOL_CNAP                              (RIL_EXT_UNSOL_RESPONSE_BASE + 29)
#define RIL_EXT_UNSOL_SIGNAL_CONN_STATUS                (RIL_EXT_UNSOL_RESPONSE_BASE + 30)
#define RIL_EXT_UNSOL_SMART_NR_CHANGED                  (RIL_EXT_UNSOL_RESPONSE_BASE + 31)
#define RIL_EXT_UNSOL_NR_CFG_INFO                       (RIL_EXT_UNSOL_RESPONSE_BASE + 32)
#define RIL_EXT_UNSOL_MODEM_STATE_CHANGED               (RIL_EXT_UNSOL_RESPONSE_BASE + 33)
#define RIL_EXT_UNSOL_RESPONSE_LAST                     RIL_EXT_UNSOL_MODEM_STATE_CHANGED
/*****************************************************************************/
...
typedef void (*RIL_HandlerTimedCallback)(RIL_TimedCallback callback,
...
typedef struct {
    ...
    RIL_HandlerTimedCallback handlerTimedCallback;
    const SE_Functions *seFunctions;
} RIL_RadioFunctions;
...
typedef struct {
    ...
    int apnTypes;               /* Supported APN types bitmap. See ApnTypes for the value of each bit. */
